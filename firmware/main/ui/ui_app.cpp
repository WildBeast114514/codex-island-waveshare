...
void UiApp::begin(AppStateStore *store, uint8_t initial_page,
    ...
    tiles_[3] = lv_tileview_add_tile(tileview_, 3, 0, static_cast<lv_dir_t>(LV_DIR_LEFT | LV_DIR_RIGHT));
    tiles_[4] = lv_tileview_add_tile(tileview_, 4, 0, LV_DIR_LEFT);
    ...
    distributed_radar_.create(tiles_[2]);
    status_.create(tiles_[3]);
    pet_.create(tiles_[4]);
...
void UiApp::refresh(int64_t monotonic_seconds) {
    ...
    usage_.update(state);
    radar_.update(state);
    distributed_radar_.update(state);
...
void UiApp::set_page(uint8_t page, bool animate) {
    const uint8_t selected =
        std::min<uint8_t>(page, static_cast<uint8_t>(kPageCount - 1));
    current_page_.store(selected);
    pet_.set_active(selected == kPageCount - 1);
    lv_tileview_set_tile_by_index(tileview_, selected, 0,
                                  animate ? LV_ANIM_ON : LV_ANIM_OFF);
}

void UiApp::next_page() {
    set_page(static_cast<uint8_t>((current_page_.load() + 1) % kPageCount));
}
...
void UiApp::tile_event(lv_event_t *event) {
    ...
    for (uint8_t i = 0; i < app->tiles_.size(); ++i) {
        if (active == app->tiles_[i]) {
            ...
            app->pet_.set_active(i == kPageCount - 1);
            break;
        }
    }
}
